...
# This script runs a version-selected RHEL-compatible Vagrant VM and executes the rdp-rhel9 Molecule
...
if [ "$(uname -m)" = "arm64" ]; then
  if [ "${RHEL_MAJOR_VERSION}" = "9" ]; then
    default_arm_box="generic/rhel9"
  else
    default_arm_box="almalinux/10.aarch64"
  fi

  if [ "${VAGRANT_DEFAULT_PROVIDER:-}" = "parallels" ] || [ "${VAGRANT_DEFAULT_PROVIDER:-}" = "qemu" ]; then
    export VAGRANT_BOX="${VAGRANT_BOX:-${default_arm_box}}"
  fi
fi

# 0) Check if vagrant is available at all
if ! command -v vagrant >/dev/null 2>&1; then
  echo "NOTE: vagrant is not installed. Skipping RHEL ${RHEL_MAJOR_VERSION} Vagrant scenario."
  exit 9
fi

# 1) VM start (but first check provider availability)
pushd vagrant/rhel >/dev/null
...
if ! vagrant up; then
  ...
  echo "      - Set RHEL_MAJOR_VERSION=9 or RHEL_MAJOR_VERSION=10 to select the OS generation."
  echo "      - Set VAGRANT_BOX to a provider/architecture-capable box you trust."
